<?php
if (!function_exists('qode_pitch_register_button')) {

    function qode_pitch_register_button($buttons) {
...
if (!function_exists('qode_pitch_add_plugin')) {

    function qode_pitch_add_plugin($plugin_array) {
...
if (!function_exists('qode_pitch_shortcodes_button')) {

    function qode_pitch_shortcodes_button() {
        if (!current_user_can('edit_posts') && !current_user_can('edit_pages')) {
            return;
        }

        if (get_user_option('rich_editing') == 'true') {
            add_filter('mce_external_plugins', 'qode_pitch_add_plugin');
            add_filter('mce_buttons', 'qode_pitch_register_button');
        }
    }

}
add_action('after_setup_theme', 'qode_pitch_shortcodes_button');

if (!function_exists('qode_pitch_remove_wpautop')){
    function qode_pitch_remove_wpautop( $content, $autop = false ) {
        if ( $autop ) {
...
if (!function_exists('no_call_to_action')) {
    ...
    function no_call_to_action($atts, $content = null) {
        ...
        foreach ($qode_pitch_IconCollections->iconCollections as $collection_key => $collection) {
        ...
        $args = array_merge($args, $qode_pitch_IconCollections->getShortcodeParams(),$call_to_action_icons_form_fields);
        ...
        if($button_hover_animation == "default" && isset($qode_pitch_options['button_hover_animation']) && ($qode_pitch_options['button_hover_animation'] !== '')){
            $animate_button = true;
        }
        elseif($button_hover_animation == "default" && isset($qode_pitch_options['button_hover_animation']) && ($qode_pitch_options['button_hover_animation'] == '') ){
            $animate_button = false;
        }
        ...
        if ($animate_button == true){
            ...
            elseif(isset($qode_pitch_options['button_hover_animation']) && ($qode_pitch_options['button_hover_animation'] != '')){
                $button_classes .= " {$qode_pitch_options['button_hover_animation']}";
            }
        }		
        ...
        $html.= '<div class="call_to_action ' . esc_attr($action_classes) . '" '.qode_pitch_get_inline_style($action_styles).'>';
        ...
        if ($show_button == 'yes') {
            if ($grid_size == "75") {
                $html .= '<div class="call_to_action_row_75_25 clearfix" '.qode_pitch_get_inline_style($action_inner_styles).'>';
            }
            elseif ($grid_size == "66") {
                $html .= '<div class="call_to_action_row_66_33 clearfix" '.qode_pitch_get_inline_style($action_inner_styles).'>';
            }
            else {
                $html .= '<div class="call_to_action_row_50_50 clearfix" '.qode_pitch_get_inline_style($action_inner_styles).'>';
            }
        ...
        if ($type == "with_icon" || $type == "with_custom_icon") {
            ...
            $html .= '<div class="call_to_action_icon_inner" '.qode_pitch_get_inline_style($icon_inner_style).'>';
            if ($custom_icon != "") {
                ...
                $html .= '<img src="' . esc_attr($custom_icon_src) . '" alt="">';
            } else {
        ...
        $html .= '<div class="call_to_action_text'. esc_attr($call_to_action_text_class) .'" '.qode_pitch_get_inline_style($content_styles).'>'.qode_pitch_remove_wpautop($content, true).'</div>';
        ...
        if ($show_button == 'yes') {
            ...
            if ($button_icon_pack != ''){
                $icon_collection_obj = $qode_pitch_IconCollections->getIconCollection($button_icon_pack);
            ...
            $html .= '<a href="' . esc_url($button_link) . '" class="qbutton '.$button_classes.' '. esc_attr($button_size) . '" '.qode_pitch_get_inline_attr($button_target, 'target').' '.qode_pitch_get_inline_style($button_styles).' ' . $data_attr . '>';
    ...
    function no_blockquote($atts, $content = null) {
        ...
        if ($show_quote_icon == 'yes') {
            if($quote_icon_font == 'with_icon'){
				if(isset($qode_pitch_options['blockquote_icon_type'])) { //if icon is set in options
					$html .= '<span style="' . implode(";", $quote_icon_styles) . '" class="icon_quotations_holder">';
					if($qode_pitch_options['blockquote_icon_type'] == '')  { //if selected icon is empty use default icon
						$html .= '<i class="qode_icon_font_awesome fa fa-quote-left "></i>';
					}
					else {
						$html .= '<i class="' . $qode_pitch_options['blockquote_icon_type'] . '"></i>';
					}					
					$html .= '</span>';
				} 
            }
...
if (!function_exists('no_button')) {
    ...
    function no_button($atts, $content = null) {
        ...
        $args = array_merge($args, $qode_pitch_IconCollections->getShortcodeParams());
...
if (!function_exists('no_counter')) {
    ...
    function no_counter($atts, $content = null) {
		...
		$default_atts = array_merge($args, $qode_pitch_IconCollections->getShortcodeParams());
		...
		if ($separator == "yes" && $separator_position == "above_title" ) {
            $html .= '<span class="separator medium" ' . qode_pitch_get_inline_style($separator_styles) . '></span>';
        }
...
//		innner div style
        ...
        $html .= '<div class="custom_font_holder" '.  qode_pitch_get_inline_style($custom_font_wrapper_styles).' '.$data_attr.'>';
		$html .= '<div '.$custom_font_inner_class.' '.qode_pitch_get_inline_style($custom_font_inner_style).'>'.qode_pitch_remove_wpautop($content, true).'</div>';
...
if (!function_exists('no_cover_boxes')) {
    ...
    function no_cover_boxes($atts, $content = null) {
        ...
        $html .= "<div class='cover_boxes ".esc_attr($items_number_class)."' data-active-element='" . $active_element . "' ".qode_pitch_get_inline_attr($items_number,"date-number_of_items")."><ul class='clearfix'>";
        ...
        if ($link_label1 != "") {
            ...
            if ($cover_boxes_icon_pack != ''){
                $icon_collection_obj = $qode_pitch_IconCollections->getIconCollection($cover_boxes_icon_pack);
...
if (!function_exists('no_icons')) {
    ...
    function no_icons($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, $qode_pitch_IconCollections->getShortcodeParams());
        ...
        $fa_size = $qode_pitch_IconCollections->getIconSizeClass($fa_size);
        ...
        $html = '<span class="q_icon_shade q_icon_shortcode ' . esc_attr($icon_pack . ' ' . $type . ' ' . $icon_stack_classes . ' ' . $hover_animation_class . ' ' . $icon_animation) . '" ' . $data_attr_stack . ' '.qode_pitch_get_inline_style($icon_stack_style.$animation_delay_style).'>';
        if ($link != "") {
            $html .= '<a href="' . esc_url($link) . '" target="' . esc_attr($target) . '" '.qode_pitch_get_inline_style($icon_link_style).' '. qode_pitch_get_class_attribute($icon_link_classes) . '>';
        }
		...
		if (($type == 'square' || $type == 'circle') && $icon_hover_animation != 'no_animation'){
			$html .= '<span class="animation_overlay" '.qode_pitch_get_inline_style($animation_icon_style).'></span>';
		}
...
if (!function_exists('no_icon_text')) {
    ...
    function no_icon_text($atts, $content = null) {
        ...
        $icon_size = $qode_pitch_IconCollections->getIconSizeClass($icon_size);
        ...
        if (($custom_icon !== "") && is_numeric($custom_icon)) {
            $custom_icon_src = wp_get_attachment_url($custom_icon);
            $custom_icon_html = '<div class="custom_icon"><img class="custom_icon_original" src=' . $custom_icon_src . ' alt="">';
			...
			if (($custom_icon_hover !== "") && is_numeric($custom_icon_hover)) {
				...
				$custom_icon_html .= '<img class="custom_icon_hover" src=' . $custom_icon_hover_src . ' alt="">';
...
if (!function_exists('no_icon_list_item')) {
    ...
    function no_icon_list_item($atts, $content = null) {
        ...
        $html .= '<p class="q_icon_list_text '.$text_class.'" '.qode_pitch_get_inline_style($title_style) . '>' . $title . '</p>';
		...
		// FRONT SIDE
			...
			if ($show_content == "always" || $show_content == "before_hover") {
				if ($content != "") {
					...
					$html .= '<p>';
					$html .= qode_pitch_remove_wpautop($content, true);
					$html .= '</p></div>';
				}
			}
...
if (!function_exists('no_image_with_text_and_icon')) {
    ...
    function no_image_with_text_and_icon($atts, $content = null) {
        ...
        foreach ($qode_pitch_IconCollections->iconCollections as $icon_set) {
            if (${$icon_set->param}) {
        ...
        $html .= '<p>' . qode_pitch_remove_wpautop($content, true) . '</p>';
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            switch ($type) {
                ...
                case 'split_column':
                    ...
                    if (has_post_thumbnail()) {
						...
						if ($image_size !== 'custom' || $image_width == '' || $image_height == ''){
							...
							$html.= qode_pitch_generate_thumbnail(get_post_thumbnail_id(get_the_ID()),null,$image_width,$image_height);
						}
                        $html .= '</a>';
                    }
                    $html .= '</div>';
                    ...
                    $html .= '<div class = "split_text_holder split_column_column" '.qode_pitch_get_inline_style($box_info_style).'>';

                    $html .= '<div class = "post_info_holder"  '.qode_pitch_get_inline_style($latest_post_info_style_array).'>';
                    if ($display_date == '1') {
                    ...
                    if ($display_category == '1') {
                        ...
                        $html .= '<div class="latest_post_categories"> '.esc_html__( 'in ','pitchwp');
                        foreach ($cat as $categ) {
                            $html .= '<a '.qode_pitch_get_inline_style($latest_post_info_link_color).' href="' . esc_url(get_category_link($categ->term_id)) . '">' . esc_html($categ->cat_name) . ' </a> ';
                        }
                    ...
                    if ($display_author == '1') {
                        ...
                        $html .= '<span '.qode_pitch_get_inline_style($latest_post_info_color).' > ' . esc_html__( "by", "pitchwp") . '</span> <a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '"><span '.qode_pitch_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span></a>';
                    ...
                    if ($display_comments == "1") {
                        ...
                        switch ($comments_count) {
                            case 0:
                                $comments_count_text = esc_html__( 'No comment', 'pitchwp');
                                break;
                            case 1:
                                $comments_count_text = $comments_count . ' ' . esc_html__( 'Comment', 'pitchwp');
                                break;
                            default:
                                $comments_count_text = $comments_count . ' ' . esc_html__( 'Comments', 'pitchwp');
                                break;
                        }
                        $html .= '<div class="latest_post_comments"> ';
                        $html .= '<a '.qode_pitch_get_inline_style($latest_post_info_link_color).' class="post_comments" href="' . esc_url(get_comments_link()) . '">';
                    ...
                    $html .= '<' . $title_tag . ' class="latest_post_title " ' . qode_pitch_get_inline_style($title_style) . '>';
                    $html .= '<a href="' . esc_url(get_permalink()) . '" '.$title_link_style.'>' . get_the_title() . '</a>';
                    $html .= '</' . $title_tag . '>';

                    if ($display_excerpt == '1' && $text_length != '0') {
                        $excerpt = ($text_length > 0) ? substr(get_the_excerpt(), 0, intval($text_length)) : get_the_excerpt();
                        $html .= '<div class = "split_column_excerpt" ' . qode_pitch_get_inline_style($excerpt_style) . '>';
                        $html .= $excerpt;
                        $html .= '...</div>';
                ...
                case 'image_in_box':
                        ...
                        $post_info_html .= '<div class="post_info_section" ' . qode_pitch_get_inline_style($latest_post_info_style_array) . '>';

                        if ($display_date == '1' && $date_place == 'by_post_info') {
                            $post_info_html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('j F Y') . ' </span>'; //close date_hour_holder
                        }
                        ...
                        if ($display_comments == "1") {
                          $comments_count = get_comments_number();
                            ...
                            $post_info_html .= '<a '.qode_pitch_get_inline_style($latest_post_info_link_color).' class="post_comments" href="' . esc_url(get_comments_link()) . '">';
                        ...
                        if ($display_category == '1') {
                            ...
                            $post_info_html .= esc_html__( 'in ','pitchwp');

                            foreach ($cat as $categ) {
                                $post_info_html .= '<a '.qode_pitch_get_inline_style($latest_post_info_link_color).' href="' . esc_url(get_category_link($categ->term_id)) . '">' . esc_html($categ->cat_name) . ' </a> ';
                            }
                        ...
                        if ($display_author == '1') {
                            ...
                            $post_info_html .= '<span '.qode_pitch_get_inline_style($latest_post_info_color).'>' . esc_html__( "by", "pitchwp") . '</span> <a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '"><span '.qode_pitch_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span></a>';
                            $post_info_html .= '</div>'; //close span.latest_post_categories
                        }
                        ...
                        $html .= '<div class="latest_post" ' . qode_pitch_get_inline_style($box_info_style) . '>';
                        ...
                        if ($display_date == '1' && $date_place == 'over_title') {
                            ...
                            $html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('j F Y') . ' </span>'; //close date_hour_holder
                            $html .= '</div>';
                        }
                        ...
                        if ($display_date == '1' && $date_place == 'by_title') {
                            $html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('d M') . ' </span>'; //close date_hour_holder
                        }
                        ...
                        if ($display_excerpt == '1' && $text_length != '0') {
                            ...
                            $html .= '<p class="excerpt" ' . qode_pitch_get_inline_style($excerpt_style) . '>' . $excerpt . '...</p>';
                        }
				...
				case 'boxes':
                        ...
                        if ($display_date == '1' && $date_place == 'by_post_info') {
                             $post_info_html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('jS F') . ' </span>'; //close date
                        }
                        ...
                        $html .= '<li class="clearfix" ' . qode_pitch_get_inline_style($minimal_style) . '>';
                        ...
                        if ($show_thumbnail == 'yes') {
                            ...
                            $html .= '<span class="latest_post_overlay" ' . qode_pitch_get_inline_style($latest_post_overlay_style) . '>';
                            if ($overlay_icon == "1") {
                        ...
                        if ($display_date == '1' && $date_place == 'over_title') {
                            $html .= '<div class="latest_post_date_over_title_holder">';
                            $html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('F d, Y') . ' </span>'; //close date_hour_holder
                            $html .= '</div>';
                        }
                    ...
                    case 'post_over_image':
                        ...
                        if ($display_comments == "1") {
                            ...
                            $post_info_html .= '<div class="latest_post_comments" '.qode_pitch_get_inline_style($latest_post_info_color).'> ';
                            $post_info_html .= '<a '.qode_pitch_get_inline_style($latest_post_info_link_color).' class="post_comments" href="' . esc_url(get_comments_link()) . '">';
                            $post_info_html .= $comments_count_text;
                            $post_info_html .= '</a></div>'; //close post_comments
                        }
                        ...
                        if ($display_category == '1') {
                            $cat = get_the_category();
                            $post_info_html .= '<div class="latest_post_categories" '.qode_pitch_get_inline_style($latest_post_info_color).'> ';
                        ...
                        $html .= '<div class="latest_post" ' . qode_pitch_get_inline_style($box_style) . '>';
                    ...
                    case 'minimal':
                        ...
                        $post_info_html .= '<div class="post_info_section" '.qode_pitch_get_inline_style($latest_post_info_style_array).'>';
                        if ($display_date == '1' && $date_place == 'by_post_info') {
                            $post_info_html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('jS F') . ' </span>'; //close date_hour_holder
                        }
                        if ($display_comments == "1") {
                            $comments_count = get_comments_number();
                            switch ($comments_count) {
                        ...
                        if ($display_author == '1') {
                            $post_info_html .= '<div class="latest_post_author">';
                            $post_info_html .= '<span '.qode_pitch_get_inline_style($latest_post_info_color).'></span> <a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '"><span '.qode_pitch_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span></a>';
                            $post_info_html .= '</div>'; //close span.latest_post_categories
                        }
						...
						if ($display_date == '1' && $date_place == 'over_title') {
                            $html .= '<div class="latest_post_date_over_title_holder">';
                            $html .= '<span class="date" ' . qode_pitch_get_inline_style($date_style) . '>' . get_the_time('jS F') . ' </span>'; //close date_hour_holder
                            $html .= '</div>';
                        }
                        $html .= '<div class="latest_post_title_holder">';
                    ...
                    case 'date_in_left_section':
                        ...
                        if ($display_like == '1') {
                            if (function_exists('qode_pitch_like_latest_posts')) {
                                $html .= '<div class="blog_like">'.qode_pitch_like_latest_posts().'</div>';
                            }
                        }
                        ...
                        if ($text_length != '0') {
                            $excerpt = ($text_length > 0) ? substr(get_the_excerpt(), 0, intval($text_length)) : get_the_excerpt();
                            $html .= '<div class = "sections_content_excerpt" ' . qode_pitch_get_inline_style($excerpt_style) . '>';
                            $html .= $excerpt;
                            $html .= '...</div>';
                        }
                    ...
                    case 'masonry':
                        ...
                        if ($display_category == '1') {
                            ...
                            $html .= '<span class="post_categories">' . esc_html__( 'In', 'pitchwp') . ' ' . implode(', ', $post_categories) . '</span>';

                        }
                        $html .= '</div>';

                        $html .= '<' . $title_tag . ' class="latest_post_title " ' . qode_pitch_get_inline_style($title_style) . '><a href="' . esc_url(get_permalink()) . '" ' . $title_link_style . '>' . get_the_title() . '</a></' . $title_tag . '>';
...
if (!function_exists('no_line_graph')) {
    ...
    function no_line_graph($atts, $content = null) {
        ...
        for ($i = 0; $i < count($line_graph_array); $i = $i + 1) {
            ...
            $color = qode_pitch_hex2rgb(trim($line_graph_el[0]));
            $html .= "{fillColor: 'rgba(" . $color[0] . "," . $color[1] . "," . $color[2] . ",0.7)',data:[" . $values . "]}";
        }
        if (!empty($qode_pitch_options['text_fontsize'])) {
            $text_fontsize = $qode_pitch_options['text_fontsize'];
        } else {
            $text_fontsize = 15;
        }
        if (!empty($qode_pitch_options['text_color']) && $custom_color == "") {
            $text_color = $qode_pitch_options['text_color'];
        } else if (empty($qode_pitch_options['text_color']) && $custom_color != "") {
...
if (!function_exists('no_progress_bar')) {
    ...
    function no_progress_bar($atts, $content = null) {
        ...
        $html .= "<{$title_tag} class='progress_title_holder clearfix' ".qode_pitch_get_inline_style($progress_title_holder_styles).">";
        ...
        if ($show_percent_number != 'no') {
            $html .= "<span class='progress_number " . $show_percent_mark . "' ".qode_pitch_get_inline_style($number_styles).">";
        }
        ...
        $html .= "<div data-percentage='" . $percent . "' class='progress_content' ".qode_pitch_get_inline_style($percentage_styles).">";
...
if (!function_exists('no_progress_bar_icon')) {
    ...
    function no_progress_bar_icon($atts, $content = null) {
        ...
        while ($i < $icons_number) {
            $html .= "<div class='bar' ".qode_pitch_get_inline_style($custom)."><span class='bar_noactive qode_icon_stack ";
            if ($size != "") {
                if ($size == "tiny") {
                    $html .= "qode_tiny_icon";
                } else if ($size == "small") {
...
if (!function_exists('no_social_share')) {
    ...
    function no_social_share($atts, $content = null) {
        ...
        if (isset($qode_pitch_options['twitter_via']) && !empty($qode_pitch_options['twitter_via'])) {
            $twitter_via = " via " . esc_attr($qode_pitch_options['twitter_via']) . " ";
        } else {
            $twitter_via = "";
        }
        ...
        if (isset($qode_pitch_options['enable_social_share']) && $qode_pitch_options['enable_social_share'] == "yes") {
            ...
            if (isset($qode_pitch_options["post_types_names_$post_type"])) {
                if ($qode_pitch_options["post_types_names_$post_type"] == $post_type) {
                    ...
                    $html .= '<span class="social_share_title">' . esc_html__( 'Share', 'pitchwp') . '</span>';
                    $html .= '</a>';
                    $html .= '<div class="social_share_dropdown"><ul>';
                    if (isset($qode_pitch_options['enable_facebook_share']) && $qode_pitch_options['enable_facebook_share'] == "yes") {
                        ...
                        if(wp_is_mobile()) {
                            ...
                            $html .= '<a href="#" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(qode_pitch_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=';
                            if (function_exists('the_post_thumbnail')) {
                                $html .= wp_get_attachment_url(get_post_thumbnail_id());
                            }
                        }
                        $html .= '&amp;p[summary]=' . urlencode(qode_pitch_addslashes(get_the_excerpt()));
                        $html .='\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');">';
                        if (!empty($qode_pitch_options['facebook_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options["facebook_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($qode_pitch_options['enable_twitter_share'] == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(qode_pitch_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(qode_pitch_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        if (!empty($qode_pitch_options['twitter_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options["twitter_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($qode_pitch_options['enable_google_plus'] == "yes") {
                        ...
                        if (!empty($qode_pitch_options['google_plus_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['google_plus_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_linkedin']) && $qode_pitch_options['enable_linkedin'] == "yes") {
                        ...
                        if (!empty($qode_pitch_options['linkedin_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['linkedin_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_tumblr']) && $qode_pitch_options['enable_tumblr'] == "yes") {
                        ...
                        if (!empty($qode_pitch_options['tumblr_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['tumblr_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_pinterest']) && $qode_pitch_options['enable_pinterest'] == "yes") {
                        $html .= '<li  class="pinterest_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a class="share_link" href="#" onclick="popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . qode_pitch_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['pinterest_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['pinterest_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_vk']) && $qode_pitch_options['enable_vk'] == "yes") {
                        ...
                        if (!empty($qode_pitch_options['vk_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['vk_icon']) . '" alt="" />';
                        } else {
...
if (!function_exists('no_social_share_list')) {
    ...
    function no_social_share_list($atts, $content = null) {
        ...
        if (isset($qode_pitch_options['enable_social_share']) && $qode_pitch_options['enable_social_share'] == "yes") {
            ...
            $share_icons_type = '';
            if(isset($qode_pitch_options['social_share_list_icons_type']) && $qode_pitch_options['social_share_list_icons_type'] !== '') {
                $share_icons_type = $qode_pitch_options['social_share_list_icons_type'];
            }
            ...
            if (isset($qode_pitch_options["post_types_names_$post_type"])) {
                if ($qode_pitch_options["post_types_names_$post_type"] == $post_type) {
                    ...
                    if ($qode_pitch_options['enable_twitter_share'] == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" title="'.__("Share on Twitter", 'qode').'" onclick="popUp=window.open(\'http://twitter.com/intent/tweet?text=' . urlencode(qode_pitch_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" title="'.__("Share on Twitter", 'qode').'" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(qode_pitch_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        if (!empty($qode_pitch_options['twitter_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options["twitter_icon"]) . '" alt="" />';
                        } else {
                    ...
                    if ($qode_pitch_options['enable_google_plus'] == "yes") {
                        $html .= '<li  class="google_share">';
                        $html .= '<a href="#" title="' . esc_html__( "Share on Google+", "pitchwp") . '" onclick="popUp=window.open(\'https://plus.google.com/share?url=' . urlencode(get_permalink()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['google_plus_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['google_plus_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_linkedin']) && $qode_pitch_options['enable_linkedin'] == "yes") {
                        $html .= '<li  class="linkedin_share">';
                        $html .= '<a href="#" class="' . esc_html__( "Share on LinkedIn", "pitchwp") . '" onclick="popUp=window.open(\'http://linkedin.com/shareArticle?mini=true&amp;url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['linkedin_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['linkedin_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_tumblr']) && $qode_pitch_options['enable_tumblr'] == "yes") {
                        $html .= '<li  class="tumblr_share">';
                        $html .= '<a href="#" title="' . esc_html__( "Share on Tumblr", "pitchwp") . '" onclick="popUp=window.open(\'http://www.tumblr.com/share/link?url=' . urlencode(get_permalink()) . '&amp;name=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['tumblr_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['tumblr_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_pinterest']) && $qode_pitch_options['enable_pinterest'] == "yes") {
                        $html .= '<li  class="pinterest_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_html__( "Share on Pinterest", "pitchwp") . '" onclick="popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . qode_pitch_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['pinterest_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['pinterest_icon']) . '" alt="" />';
                        } else {
                    ...
                    if (isset($qode_pitch_options['enable_vk']) && $qode_pitch_options['enable_vk'] == "yes") {
                        $html .= '<li  class="vk_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_html__( "Share on VK", "pitchwp") . '" onclick="popUp=window.open(\'http://vkontakte.ru/share.php?url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '&amp;image=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (!empty($qode_pitch_options['vk_icon'])) {
                            $html .= '<img src="' . esc_url($qode_pitch_options['vk_icon']) . '" alt="" />';
                        } else {
...
if (!function_exists('no_team')) {
    ...
    function no_team($atts, $content = null) {
        ...
        if ($team_image_hover_color != "") {
            ...
            $final_color = qode_pitch_rgba_color($team_image_hover_color, $opacity);
			...
			case "on_hover":
				...
				if ($team_image != "") {
					...
					if ($team_social_icon_pack !== '') {
						$icon_collection_obj = $qode_pitch_IconCollections->getIconCollection($team_social_icon_pack);
                    ...
                    if($link != '') {
                        $button_param_array = '';
                        $html .= do_shortcode('[no_button  link="' . $link . '" target="' . $link_target .  '" icon_pack="font_elegant" fe_icon="arrow_right" icon_position="right" text="'.esc_html__( "Read More", "pitchwp").'"]');
                    }
			...
			case "below_image":
				...
				if ($team_image != "") {
					...
					if ($team_social_icon_pack !== '') {
						...
						$html .= "<div class='q_team_social_holder_between " .$team_hover_type. " " .$social_icons_position. "' ".  qode_pitch_get_inline_style($social_image_bottom_style).">";
...
if (!function_exists('no_image_slider_no_space')) {
    ...
    function no_image_slider_no_space($atts, $content = null) {
        ...
        if (isset($qode_pitch_options['carousel_navigation_arrows_type']) && $qode_pitch_options['carousel_navigation_arrows_type'] != '') {
            $icon_navigation_class = $qode_pitch_options['carousel_navigation_arrows_type'];
        }
...
if (!function_exists('no_separator_with_icon')) {
    ...
    function no_separator_with_icon($atts, $content = null) {
        ...
        $icon_position = '50';
        if ($type == 'with_custom_icon' && $custom_icon != '') {
            $custom_icon_dimensions_array = qode_pitch_get_image_dimensions($custom_icon_src);
        ...
        $html .= '<div class="' . esc_attr($separator_classes) . '" '.qode_pitch_get_inline_style($separator_styles).'>';
        ...
        elseif($type == 'with_custom_icon'){
			if($link != "") {
				$html .= '<div class="separator_custom_icon"><a href="'.esc_url($link).'" target="'.esc_attr($target).'" '.$icon_link_classes.'><img src="' . esc_url($custom_icon_src) . '" alt="" /></a></div>';
			}
			else {
				$html .= '<span><img src="' . esc_url($custom_icon_src) . '" alt="" /></span>';
			}
		}
...
if (!function_exists('no_blog_carousel')) {
    ...
    function no_blog_carousel($atts, $content = null) {
        ...
        if ($blog_query->have_posts()) : $postCount = 0;
            while ($blog_query->have_posts()) : $blog_query->the_post();
                ...
                switch($type) {
                    case 'info_in_bottom':
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_info_in_bottom" ' . qode_pitch_get_inline_style($hover_box_style) . '>';
                        ...
                        if ($show_date == 'yes' || $show_author=='yes' || $show_categories == 'yes') {
                            ...
                            $html.= '<div class = "blog_slider_post_info" ' . qode_pitch_get_inline_style($post_info_style) . '>';
                            ...
                            if ($show_author == 'yes') {
                                $html .= '<span class="blog_slider_author_holder">' . esc_html__( "by ", "pitchwp") . '<a href="' . get_author_posts_url(get_the_author_meta("ID")) . '" ' . qode_pitch_get_inline_style($author_style) . '>' . get_the_author_meta("display_name") . '</a></span>';
                            }

                            if ($show_categories == 'yes') {
                                $html .= '<div class="blog_slider_categories">' . esc_html__( "in ", "pitchwp");
                    ...
                    default:
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_default ' . $info_type_class . '" ' . qode_pitch_get_inline_style($hover_box_style) . '>';
                        $html .= '<div class="blog_text_holder_outer">';
                        ...
                        if ($show_separator == 'yes') {
                        	$html .= '<span class="blog_slider_separator" '. qode_pitch_get_inline_style($separator_style) . '></span>';
                        }
            ...
            $html .= esc_html__( 'Sorry, no posts matched your criteria.', 'pitchwp');
        endif;
            ...
            while ($blog_query->have_posts()) : $blog_query->the_post();
				...
				if ($type == 'bottom'){
					...
					if($show_read_more == "yes"){
						...
						$html .= '<a href="'.get_the_permalink().'" target="_self" class="qbutton small read_more_button">'.esc_html__( "Read More", "pitchwp").'</a>';				
