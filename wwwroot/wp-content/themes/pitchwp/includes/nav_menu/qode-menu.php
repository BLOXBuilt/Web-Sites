...
add_filter( 'wp_setup_nav_menu_item', 'qode_pitch_add_custom_nav_fields');

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'qode_pitch_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'qode_pitch_edit_walker', 10, 2 );


function qode_pitch_add_custom_nav_fields( $menu_item ) {
 ...
 * @return      void
*/
function qode_pitch_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
 ...
 * @return      void
*/
function qode_pitch_edit_walker($walker,$menu_id) {

	return 'Qode_Pitch_Walker_Nav_Menu_Edit_Custom';
		
}

include_once get_template_directory().'/includes/nav_menu/edit_custom_walker.php';


/* Custom WP_NAV_MENU function for top navigation */

if (!class_exists('qode_pitch_type1_walker_nav_menu')) {
	class qode_pitch_type1_walker_nav_menu extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$wide_menu_background = isset($qode_pitch_options['enable_wide_manu_background']) && $qode_pitch_options['enable_wide_manu_background'] == 'yes';
			
			$indent = str_repeat("\t", $depth);
			if($depth == 0){
				$out_div = '<div class="second'.$background_image_class.'" '.($wide_menu_background ? qode_pitch_get_inline_style($menu_image_background) : '').'><div class="inner">';
			}else{
				$out_div = '';
			}

			// build html
			$output .= "\n" . $indent . $out_div  .'<ul '.(!$wide_menu_background ? qode_pitch_get_inline_style($menu_image_background) : '').'>' . "\n";
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if(isset($qode_pitch_options['enable_wide_manu_background']) && $qode_pitch_options['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && isset($qode_pitch_options['enable_dropdown_menu_item_icon']) && $qode_pitch_options['enable_dropdown_menu_item_icon'] == "yes" ){
				...
				if(isset($qode_pitch_options['menu_item_hover_animation']) && $qode_pitch_options['menu_item_hover_animation'] == "roll" && $depth == 0){
...
if (!class_exists('qode_type2_walker_nav_menu')) {
	class qode_pitch_type2_walker_nav_menu extends Walker_Nav_Menu {
...
if (!class_exists('qode_pitch_type3_walker_nav_menu')) {
    class qode_pitch_type3_walker_nav_menu extends Walker_Nav_Menu {
...
if (!class_exists('qode_pitch_type4_walker_nav_menu')) {
    class qode_pitch_type4_walker_nav_menu extends Walker_Nav_Menu {
