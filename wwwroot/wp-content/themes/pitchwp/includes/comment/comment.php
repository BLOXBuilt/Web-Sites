...
if (!function_exists('qode_pitch_comment')) {
function qode_pitch_comment($comment, $args, $depth) {
	$GLOBALS['comment'] = $comment; 

	global $qode_pitch_options, $post;
	$title_tag="h5";
	...
	<li>
		<div class="<?php echo esc_attr($comment_class); ?>">
			<?php if(!$is_pingback_comment) { ?>
				<div class="image"> <?php echo qode_pitch_kses_img(get_avatar($comment, 102)); ?> </div>
			<?php } ?>
			<div class="text">
				<div class="comment_info">
					<<?php echo esc_attr($title_tag);?> class="name">
						<?php if($is_pingback_comment) { esc_html_e('Pingback:', 'pitchwp'); } ?>
				...
				<?php if(!$is_pingback_comment) { ?>
					...
					<span class="comment_date"><?php comment_time(get_option('date_format')); ?> <?php esc_html_e('at', 'pitchwp'); ?> <?php comment_time(get_option('time_format')); ?></span>
				<?php } ?>
			</div>
		</div>
