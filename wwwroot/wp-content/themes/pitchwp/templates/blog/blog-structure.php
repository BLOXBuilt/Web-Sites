<?php
	...
	$sidebar = $qode_pitch_options['category_blog_sidebar'];
	...
	if(isset($qode_pitch_options['blog_masonry_choose_type'])){
		$blog_masonry_type = $qode_pitch_options['blog_masonry_choose_type'];
	} 

	$blog_list = "";
	if($qode_pitch_template_name != "") {
		if($qode_pitch_template_name == "blog-masonry.php"){
		...
		}elseif($qode_pitch_template_name == "blog-masonry-full-width.php"){
			if($blog_masonry_type == "blog_masonry_standard"){
				$blog_list = "blog_masonry";
		...
		}elseif($qode_pitch_template_name == "blog-standard.php"){
            $blog_list = "blog_standard";
            $blog_list_class = "blog_standard_type";
        }elseif($qode_pitch_template_name == "blog-standard-info-on-side.php"){
			$blog_list = "blog_standard_info_on_side";
			$blog_list_class = "blog_standard_type";
		}elseif($qode_pitch_template_name == "blog-standard-whole-post.php"){
    ...
    if(isset($qode_pitch_options['pagination_masonry'])){
       $pagination_masonry = $qode_pitch_options['pagination_masonry'];
    ...
    if(isset($qode_pitch_options['pagination_masonry_gallery'])){
       $pagination_masonry_gallery = $qode_pitch_options['pagination_masonry_gallery'];
    }
?>
<?php
	...
	if (isset($qode_pitch_options['blog_masonry_columns']) && $qode_pitch_options['blog_masonry_columns'] !== '') {
		$blog_masonry_columns = $qode_pitch_options['blog_masonry_columns'];
	}

	$blog_masonry_full_width_columns = 'five_columns';
	if (isset($qode_pitch_options['blog_masonry_full_width_columns']) && $qode_pitch_options['blog_masonry_full_width_columns'] !== '') {
		$blog_masonry_full_width_columns = $qode_pitch_options['blog_masonry_full_width_columns'];
	}

	
	if($qode_pitch_template_name == "blog-masonry.php" || $blog_style == 'blog_masonry'){
		$blog_list_class .= " " .$blog_masonry_columns;
	}
	
	if($qode_pitch_template_name == "blog-masonry-full-width.php" || $blog_style == 'blog_masonry_full_width'){
		$blog_list_class .= " " .$blog_masonry_full_width_columns;
	}

	
	$icon_left_html =  "<i class='pagination_arrow arrow_carrot-left'></i>";
	if (isset($qode_pitch_options['pagination_arrows_type']) && $qode_pitch_options['pagination_arrows_type'] != '') {
		$icon_navigation_class = $qode_pitch_options['pagination_arrows_type'];
	...
	if (isset($qode_pitch_options['blog_pagination_previous_label']) && $qode_pitch_options['blog_pagination_previous_label'] != '') {
		$icon_left_html.= $qode_pitch_options['blog_pagination_previous_label'];
	}
	else{
	...
	if (isset($qode_pitch_options['blog_pagination_next_label']) && $qode_pitch_options['blog_pagination_next_label'] != '') {
		$icon_right_html .= $qode_pitch_options['blog_pagination_next_label'];
	}
	else {
	...
	<?php endwhile; ?>
	<?php if($blog_list != "blog_masonry" && $blog_list != "blog_masonry_meta_info_featured_on_side") {
		if ($qode_pitch_options['blog_pagination_type'] == 'standard'){
				qode_pitch_pagination($blog_query->max_num_pages, $blog_page_range, $paged);
			}
		elseif ($qode_pitch_options['blog_pagination_type'] == 'prev_and_next'){?>
	...
	<div class="entry">
			<p><?php esc_html_e('No posts were found.', 'pitchwp'); ?></p>
	</div>
	<?php endif; ?>
</div>
<?php if($blog_list == "blog_masonry" || $blog_list == "blog_masonry_meta_info_featured_on_side") {
    if($pagination_masonry == "load_more") {
		if (get_next_posts_link(null, $blog_query->max_num_pages)) { ?>
			<div class="blog_load_more_button_holder">
				<div class="blog_load_more_button"><span data-rel="<?php echo esc_attr($blog_query->max_num_pages); ?>"><?php echo wp_kses_post(get_next_posts_link(esc_html__('Show more', 'pitchwp'), $blog_query->max_num_pages)); ?></span></div>
			</div>
		<?php } ?>
	 <?php } elseif($pagination_masonry == "infinite_scroll") { ?>
		<div class="blog_infinite_scroll_button"><span data-rel="<?php echo esc_attr($blog_query->max_num_pages); ?>"><?php echo wp_kses_post(get_next_posts_link(esc_html__('Show more', 'pitchwp'), $blog_query->max_num_pages)); ?></span></div>
    <?php }else { ?>
        <?php if($qode_pitch_options['blog_pagination_type'] == 'standard' && $qode_pitch_options['pagination'] != "0") {
				qode_pitch_pagination($blog_query->max_num_pages, $blog_page_range, $paged);
            }
        	elseif ($qode_pitch_options['blog_pagination_type'] == 'prev_and_next'){ ?>
				<div class="pagination_prev_and_next_only">
