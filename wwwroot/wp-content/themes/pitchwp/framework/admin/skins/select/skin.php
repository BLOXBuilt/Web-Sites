...
class SelectPitchSkin extends QodePitchSkinAbstract {
    /**
     * Skin constructor. Hooks to qode_pitch_admin_scripts_init and qode_pitch_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('qode_pitch_admin_scripts_init', array($this, 'registerStyles'));
        add_action('qode_pitch_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('qode_pitch_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('qode_pitch_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('qode_pitch_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('qode_pitch_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            qode_pitch_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            qode_pitch_register_skin_style($styleHandle, $stylePath);
        }
	...
	public function renderOptions() {
        ...
        $tab    = qode_pitch_get_admin_tab();
        $active_page = $qode_pitch_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see QodePitchSkinAbstract::loadTemplatePart()
     */
