...
function callCallback(callbacks, name, self, args, variables) {
    ...
    qodefAnimateOverlappingContent(); //needs to be in callback to wait all other functions to execute and than to fade in the content
}
	...
	if(hash !== undefined && $j('[data-q_id="'+hash+'"]').length > 0){
		...
		if($window_width > 1000){
			if($j('header.page_header').hasClass('fixed') && !$j('body').hasClass('vertical_menu_enabled')){
				...
				if(!$j('header.page_header').hasClass('transparent') || $j('header.page_header').hasClass('scrolled_not_transparent')){
					scrollToAmount = $j('[data-q_id="' + hash + '"]').offset().top -  header_height - large_menu_item_border - paspartuScrollAdd;
				}else{
					scrollToAmount = $j('[data-q_id="' + hash + '"]').offset().top - paspartuScrollAdd;
				}
...
function loadResource(url) {
    ...
    var me = $j("nav a[href='"+qode_root+url+"'], .widget_nav_menu a[href='"+qode_root+url+"']");
    ...
    $j.ajax({
        ...
        statusCode: {
            404: function() {
                //This will display 404 page and reload website
				//Other way 404 would never be displayed
				document.location.href = qode_root + url;
            }
        }
    });
