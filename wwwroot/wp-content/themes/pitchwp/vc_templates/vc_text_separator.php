...
extract(shortcode_atts(array(
    'title' => esc_html__("Title", "pitchwp"),
    'title_size' => '',
    'title_color' => '',
    'text_in_box' => 'yes',
...
if($box_height != ""){
    ...
    $separator_wrapper_styles[] = "line-height:". $box_height;
}
...
$output .= '<span data-line-width="'.$line_width.'"  class="'.$css_class . $animation_class.' '.$text_position.'">';
    $output .= '<span class="separator_wrapper" style="'.implode(';', $separator_wrapper_styles).'">';
    if ($text_position == 'center' || $text_position == 'right') {
        $output .= '<span class="q_line_before '.$separator_line_classes.'"  style="'.implode(';', $separator_lines_styles).';'.$line_right_margin.'">';
    }
    if($line_dots == "yes") {
        $output .= '<span class="q_separator_line_inner" style="' . implode(';', $separator_line_dots_style) . ';' . $separator_line_left_dot_position . '"></span>';
    }
    if ($text_position == 'center' || $text_position == 'right') {
        $output .= '</span>'; // end q_line_before
    }
    $output .= '<span class="separator_content '.$separator_box_classes.'" style="'.implode(';', $separator_box_styles).'">';
        $output .= '<span>' . $title . '</span>';
    $output .='</span>'; // end separator_content
    if ($text_position == 'center' || $text_position == 'left') {
        $output .= '<span class="q_line_after '.$separator_line_classes.'" style="'.implode(';', $separator_lines_styles).';'.$line_left_margin.'">';
    }
    if($line_dots == "yes") {
        $output .= '<span class="q_separator_line_inner" style="' . implode(';', $separator_line_dots_style) . ';' . $separator_line_right_dot_position . '"></span>';
    }
    if ($text_position == 'center' || $text_position == 'left') {
        $output .= '</span>'; // end q_line_after
    }
    $output .='</span>'; // end separator_wrapper
$output .= '</span>'.$this->endBlockComment('separator')."\n"; // end separator_content
