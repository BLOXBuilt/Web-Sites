...
$args = array(
	...
	'pattern_background' => '',
	'floating_image' => '',
	'floating_direction' => 'top-left-bottom-right',
	'floating_speed' => '3',
	'floating_zigzag' => 'yes',
	'floating_randsize' => 'yes',
	'border_color' => '',
	'border_top_color' => '',
	'padding_top' => '',
	'padding_bottom' => '',
...
$args = array_merge($args, $qode_pitch_IconCollections->getShortcodeParams('content_menu_'));
...
if ($icon_pack !== '' && $in_content_menu !== '') {
    ...
    $item_collection_icon_obj = $qode_pitch_IconCollections->getIconCollection($icon_pack);
...
$initial_button_label = $more_button_label;
$set_as_expanded_class = "";
$arrow_class = "";
if($set_as_expanded == "set_as_expanded"){
	$set_as_expanded_class = "more_fact_opened";
	$initial_button_label = $less_button_label;
	$arrow_class = 'rotate_arrow';
}
...
if($row_type == 'row') {
			...
			$output .= " style='";
				...
				if($border_color != ""){
					if($use_as_box == 'use_row_as_box') {
						...
						if($row_box_border_width != ""){
			                $border_width = (strstr($row_box_border_width, 'px', true)) ? $row_box_border_width : $row_box_border_width . "px";										
						}
						else {
							$border_width = '1px';
						}
						$output .=" border: ".$border_width." solid ".$border_color.";";
					}else {
	...
	if ($floating_image !== '') {
		$the_floating_image = wp_get_attachment_image_src($floating_image);
		$output .= 
			'<div class="section-background-elements" data-dir="' . $floating_direction . '" data-zigzag="' . $floating_zigzag . '" data-randsize="' . $floating_randsize . '" data-floaterspeed="' . $floating_speed . '" data-floaterwidth="' . $the_floating_image[1] . '" data-floaterheight="' . $the_floating_image[2] . '" data-floatersrc="' . $the_floating_image[0] . '">' .
				'<div class="section-background-elements-inner s-bei-1"></div>' .
				'<div class="section-background-elements-inner s-bei-2"></div>' .
			'</div>'
			;
	}
	...
	$output.=">";
	$output .= '<div class="more_facts_holder '.$set_as_expanded_class . '"';
	...
	$output .='><span class="more_facts_button_text">'. $initial_button_label .'</span><span class="more_facts_button_arrow ' . $arrow_class . '"><i class="fa fa-angle-down"></i></span></span>';
	$output .= '</div>';
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		$output .= '<div class="logo">';
			...
			if(isset($qode_pitch_options['content_menu_logo_image']) && $qode_pitch_options['content_menu_logo_image'] != ""){
				$logo_image = $qode_pitch_options['content_menu_logo_image'];
			}else if(isset($qode_pitch_options['logo_image']) && $qode_pitch_options['logo_image'] != ""){
				$logo_image = $qode_pitch_options['logo_image'];
			}
	...
	if($custom_widget_area !== ''){
		$output .= '<div class="custom_widget_area"><div class="custom_widget_area_inner"><div class="custom_widget_area_inner2">';
			$output .= qode_pitch_get_dynamic_sidebar($custom_widget_area);
		$output .= '</div></div></div>';
	}
