...
class Qode_Pitch_Import {
    ...
    function init_qode_import() {
        if(isset($_REQUEST['import_option'])) {
            ...
            if($import_option == 'content'){
                ...
                $this->import_menus('menus.txt');
                $this->import_settings_pages('settingpages.txt');
                $this->message = esc_html__("Content imported successfully", "pitchwp");
            }
        }
    }
    ...
    public function import_content($file){
        if (!class_exists('WP_Importer')) {
            ...
            if(is_wp_error($returned_value)){
                $this->message = esc_html__("An Error Occurred During Import", "pitchwp");
            }
            else {
                $this->message = esc_html__("Content imported successfully", "pitchwp");
            }
            ...
            $this->message = esc_html__("Error loading files", "pitchwp");
        }
    }
    ...
    public function import_widgets($file, $file2){
        ...
        $this->message = esc_html__("Widgets imported successfully", "pitchwp");
        $this->message = esc_html__("Widgets imported successfully", "pitchwp");
    }
    ...
    public function import_custom_sidebars($file){
        ...
        $this->message = esc_html__("Custom sidebars imported successfully", "pitchwp");
    }

    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'qode_options_pitch', $options);
        $this->message = esc_html__("Options imported successfully", "pitchwp");
    }
    ...
    public function import_settings_pages($file){
        ...
        foreach($pages as $qode_page_option => $qode_pitch_page_id){
            update_option( $qode_page_option, $qode_pitch_page_id);
        }
    }
    ...
    function qode_admin_import() {
        ...
        if(qode_pitch_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($qode_pitch_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-' . $this->pagehook, 'qode_pitch_enqueue_admin_scripts');
            add_action('admin_print_styles-' . $this->pagehook, 'qode_pitch_enqueue_admin_styles');
...
function qode_init_import_object(){
    global $qode_pitch_import_object;
    $qode_pitch_import_object = new Qode_Pitch_Import();
}
...
if(!function_exists('qode_dataImport')){
    function qode_dataImport(){
        ...
        if ($_POST['import_attachments'] == 1)
            $qode_pitch_import_object->attachments = true;
        else
            $qode_pitch_import_object->attachments = false;

        $folder = "pitch/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $qode_pitch_import_object->import_content($folder.$_POST['xml']);
...
if(!function_exists('qode_widgetsImport')){
    function qode_widgetsImport(){
        ...
        $qode_pitch_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
...
if(!function_exists('qode_optionsImport')){
    function qode_optionsImport(){
        ...
        $qode_pitch_import_object->import_options($folder.'options.txt');
...
if(!function_exists('qode_otherImport')){
    function qode_otherImport(){
        ...
        $qode_pitch_import_object->import_options($folder.'options.txt');
        $qode_pitch_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $qode_pitch_import_object->import_menus($folder.'menus.txt');
        $qode_pitch_import_object->import_settings_pages($folder.'settingpages.txt');
