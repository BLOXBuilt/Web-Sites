...
class PortfolioList implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
            ...
            if(qode_pitch_core_theme_installed()) {
                $icons_array = $qode_pitch_IconCollections->getVCParamsArray(array("element" => "show_load_more_icon", "value" => array("yes")));
            }
            ...
            vc_map( array(
                ...
                "params" => array_merge(array(
						...
						array(
                            ...
                            "holder" => "div",
                            "class" => "",
                            "heading" => "Enable Read More Button",
                            "param_name" => "read_more_button",
                            "value" => array(
                                "No" => "no",
                                "Yes" => "yes"
                            ),
                            "description" => "Display button with 'Check it out' text linking to portfolio page. This option is available only for Text on Image Hover (No Space) template with Subtle Vertical animation.",
                        ...
                        array(
                            ...
                            "description" => "Note: Image link will always be disabled if you enable icons in: 'Subtle Vertical', 'Image Subtle Rotate & Zoom', 'Image & Text Zoom', 'Slide Up', 'Circle' and 'Animated Border' hover animation type"
                        ),
                        array(
                            "type" => "dropdown",
                            "holder" => "div",
                        ...
                        array(
                            ...
                            "value" => "",
                            "description" =>  esc_html__("Please insert top margin for load more button", 'qode_core'),
    ...
    public function render($atts, $content = null) {
        ...
        if (isset($qode_pitch_options['portfolio_qode_like'])) {
            $portfolio_qode_like = $qode_pitch_options['portfolio_qode_like'];
        }

        $portfolio_filter_class = "";
        if (isset($qode_pitch_options['portfolio_filter_disable_separator']) && !empty($qode_pitch_options['portfolio_filter_disable_separator'])) {
            if ($qode_pitch_options['portfolio_filter_disable_separator'] == "yes") {
        ...
        if (isset($qode_pitch_options['paspartu_portfolio_full_width']) && $qode_pitch_options['paspartu_portfolio_full_width'] =='yes' ){
        ...
        if ($overlay_background_color != '' || $gradient_position != '') {
            if ($hover_type == "gradient_hover") {
                if (substr($overlay_background_color, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                    ...
                    $rgb = qode_pitch_hex2rgb($overlay_background_color);
        ...
        if ($type != 'masonry') {
            ...
            if ($filter == "yes") {
                ...
                if ($disable_filter_title != "yes") {
                    $html .= "<li class='filter_title'><span>" . esc_html__('Sort Portfolio:', 'qode_core') . "</span></li>";
                }
                if ($type == 'masonry_with_space' || $type == 'masonry_with_space_without_description') {
                    $html .= "<li class='filter' data-filter='*'><span>" . esc_html__('All', 'qode_core') . "</span></li>";
                } else {
                    $html .= "<li class='filter' data-filter='all'><span>" . esc_html__('All', 'qode_core') . "</span></li>";
                }
            ...
            if (have_posts()) : while (have_posts()) : the_post();
                    ...
                    if ($overlay_background_color_custom != '' || $gradient_position != '') {
                        if ($hover_type == "gradient_hover") {
                            if (substr($overlay_background_color_custom, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                                ...
                                $rgb = qode_pitch_hex2rgb($overlay_background_color_custom);
                ...
                $category_html = "";
                /* $category_html .= '<span>'. esc_html__('In ', 'qode_core') .'</span>'; */
                $k = 1;
                foreach ($terms as $term) {
                        ...
                        if ($disable_text_holder != 'yes' || $show_icons == 'yes' || $hover_type == 'thin_plus_only') {
                            ...
                            if ($show_icons == 'yes') {
                                ...
                                if ($portfolio_qode_like == "on" && $show_like == "yes") {
                                    if (function_exists('qode_pitch_like_portfolio_list')) {
                                        $html .= qode_pitch_like_portfolio_list(get_the_ID());
                                    }
                                }
                                if ($link_icon == "yes") {
                ...
                if ($type == "standard" || $type == "standard_no_space" || $type == "masonry_with_space") {
                    $html .= "<div class='portfolio_description " . $portfolio_description_class . "' ".qode_pitch_get_inline_style(implode('; ', $portfolio_box_style)).">";
            ...
            endwhile;
                ...
                ?>
                <p><?php _e('Sorry, no posts matched your criteria.', 'qode_core'); ?></p>
            <?php
            endif;
            ...
            if (get_next_posts_link()) {
                if ($show_load_more == "yes" || $show_load_more == "") {
                    $html .= '<div class="portfolio_paging" ' . $show_more_style . '><span data-rel="' . $wp_query->max_num_pages . '" class="load_more">' . get_next_posts_link(esc_html__('Show more'.$add_icon, 'qode_core')) . '</span></div>';
                    $html .= '<div class="portfolio_paging_loading" ' . $show_more_style . '><a href="javascript: void(0)" class="qbutton">' . esc_html__('Loading...', 'qode_core') . '</a></div>';
                }
            }
